...
def paths_for_existing_session(root: str | Path) -> SessionPaths:
    """Return a read-only compatibility view for an existing session."""
    session_dir = Path(root).expanduser().resolve()
    from eegle.recording.session import Session

    session = Session.open(session_dir, read_only=True, allow_legacy=True)
    return SessionPaths.from_session(session)
