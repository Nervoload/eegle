...
signal another process. The post-P7-012 correctness slice now scopes recording
sinks by phase, verifies complete processing-contract transformations, maps
canonical provenance field by field, expands phase goals through dependencies,
and exercises full simulated action/replay plus independent model algorithms.
The compositional implementation is split across domain, composition,
lowering, provenance, schema, and project-output modules.

P7-013 is in progress. Six exact compositional preset revisions, normalized
design input for `eegle new`, six self-indexed reference projects, an
independent example-model plugin distribution, and a dependency-lazy MNE bridge
are implemented. Exact template `1.0.0` revisions remain unchanged. The real
EEG LSL observe-only acceptance gate remains open and is not represented by
simulated evidence. The verified v1 foundation provides:
...
Create an exact compositional preset or consume a normalized design directly:

```bash
eegle new observer --id observer \
  --preset eegle.preset.event_locked_observation \
  --preset-version 2.0.0

eegle new imported --id imported --design experiment.design.json
```

The six materialized examples live under `reference_projects/`. Presets write
an immutable preset lock beside the normalized design and never replace an
exact template revision silently.
