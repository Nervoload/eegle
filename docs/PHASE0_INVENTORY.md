...
| Phase 7 — authoring, operations, packaging, and integrations | `eegle/devices/**`, `eegle/hardware/**`, `eegle/integrations/**`, `eegle/{lsl,preflight,eeg_csv,psychopy_input,recording_health}.py`, `eegle/tasks/**`, `eegle/workers/**`, site/hardware portions of `configs/**`, and the remaining root application modules | New authoring/operations services, CLI, and first-party adapter packages own project/deployment generation, preflight, source/outlet discovery, task-framework boundaries, process health, and site-local deployment without importing the old application. |
