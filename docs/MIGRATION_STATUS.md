...
| Field | Current value |
|---|---|
| Active phase | Phase 8 — validation and qualification; Phase 7 external closure evidence remains open |
| Phase status | Active; P8-001's correctness foundation, hardware-independent live hardening, and the first P8-002 research-integration adapters/support matrix are implemented, while comprehensive scientific validation, qualification, and alpha gates remain open |
...
Phase 8 implementation can proceed without fabricating the two external Phase
7 records. [PHASE8_VALIDATION_FOUNDATION.md](PHASE8_VALIDATION_FOUNDATION.md)
documents the first vertical slice: versioned result/observation/evidence-
reference contracts, a compiler/runtime metric registry, explicit insufficient
evidence, semantic record decoding, current replay taxonomy, a read-only
`validate` service/CLI, incremental application-run evidence, hardware-free
compiled LSL lifecycle evidence, explicit raw-clock mapping, and LSL-shaped MNE
raw/annotation/epoch/replay adapters. A packaged support matrix separates
representability, adapter availability, validation, and reference support.
Broader scientific metrics, scale/fault budgets, retained installed-artifact
qualification, and alpha release gates remain Phase 8 work.
...
| K-015 | **Resolved 2026-07-29:** example mean, peak, and adaptive models return the same finite explicit abstention for empty and all-invalid windows, while partially valid windows admit only finite valid samples. | Reusable conformance cases and fresh-component replay make invalid-input behavior a deterministic plugin acceptance claim. | Phase 7 P7-013A |
| K-016 | **Resolved 2026-07-29:** compilation accepts the complete authored/deployment input before staging a full generated revision, publishes that directory atomically, and switches the manifest last. | A regression test proves failed compilation preserves the prior index and generated bytes. | Phase 7 P7-013B |
| K-017 | **Resolved for the Phase 7 candidate 2026-07-29:** distribution metadata supplies Python/CLI identity; API/install/status/URL prose is current; wheel and sdist boundaries are checked from built artifacts. | Phase 8 still owns TestPyPI rehearsal, final identity audit, and public-alpha promotion. | Phase 7 P7-013B and Phase 8 release gate |
...
| P7-010 | in progress | Implement the optional first-party LSL source/event/outlet and discovery path. | Dense/sparse/metadata sources and outlets, exact detection/selectors, full native channel-descriptor retrieval, raw timestamps plus recorded boundary-clock mapping, engine-level silence/reconnect/gap/cancellation/graceful-completion/duration/final-persistence evidence, blocked-import isolation, simulated portability, and installed-wheel native local-network packet acceptance pass. Truthful support remains `simulated_validated` until the documented real EEG observe-only run is performed. |
...
| P7-013A | done | Generalize the plugin-developer workflow before public-surface closure. | Safe descriptor `inspect`/`check` commands and a reusable installed-wheel harness cover independent entry-point discovery, explicit construction, lifecycle, state restore, cleanup/failure, execution/replay equivalence, and deterministic finite handling of valid/empty/partially invalid/all-invalid model inputs without connecting to hardware during inspection. |
| P7-013B | done | Make project publication and release identity internally consistent before closure. | Failed compilation preserves the last complete generated set; successful generated specs publish as one content-addressed revision before the atomic manifest switch; distribution metadata is the single version authority; current API docs replace legacy claims; clean wheel/sdist installs prove shipped imports and commands; the sdist excludes migration sources; release jobs isolate base, extras, plugins, references, and artifact smoke failures; URLs and pre-alpha language are truthful. |
| P7-014 | in progress | Close Phase 7 against every user journey and normative exit gate. | All fifteen gates and eleven closure requirements are mapped in `PHASE7_CLOSURE.md` and its machine record; local artifact, native optional, model, plugin, non-model, simulated-action, transaction, surface, and identity proofs pass. Only the real EEG observe-only record and current-candidate remote workflows remain external blockers. |
| P8-001 | in progress | Implement comprehensive validation as a public read-only service and `eegle validate` command. | The first vertical slice packages typed schemas/statuses, compiler/runtime metric registry, insufficient-evidence acceptance, semantic evidence registry, replay taxonomy, and read-only project/session aggregation across definition, compatibility, integrity, timing, causality, execution, model, adaptation, action, protocol, and replay. Scientific performance and deeper coverage validators remain. |
| P8-002 | in progress | Qualify research integrations and publish the support matrix. | Dependency-lazy MNE adapters now cover regular/explicit dense raw, marker annotations, admitted-window epochs, and explicit-clock raw-plus-annotation replay inputs with EEGle timing/lineage sidecars. A packaged, hash-verified support matrix separates representable, adapter-available, validated, and reference-supported claims. The expanded installed-wheel native-LSL/MNE workflow awaits a retained current result; pyRiemann/foundation-model paths remain representable but unimplemented and unclaimed. |
| P8-003 | in progress | Complete performance, scale, and fault qualification. | A packaged hash-verified profile now publishes compact-engine memory/throughput/latency, durable-capture throughput, and queue-accounting budgets. Core campaigns distinguish recoverable truncation from corrupt/missing artifacts and cover silence, reconnect, gaps, clock drift, and reject-newest backpressure. Profiling justified compact phase-result retention and constant-time queue counters. High-channel/sparse scale, process-proxy, repeated-fault, and multi-process restart qualification remain. |
| P8-004 | todo | Harden and publish the truthful public alpha. | Exact wheel/sdist candidates pass TestPyPI installation, license/dependency/typing/import/API audits, current tutorials and release notes, stability/deprecation policy, immutable identity checks, and final promotion only after all Phase 8 gates pass. |
...
| R-020 | resolved | The supported-version GitHub matrix previously failed the clean-wheel journey because it disabled PEP 517 build isolation and relied on ambient `setuptools`. | Base and example-plugin wheels now use isolated builds with their declared backends, workflow actions use supported majors, and the failed workflow smoke path has been corrected. P7-014 still records the successful supported remote matrix as ordinary closure evidence. |
| R-021 | resolved | Non-transactional project recompilation could publish canonical files that do not match the last manifest after compiler failure. | P7-013B publishes content-addressed complete generated revisions only after compiler acceptance and atomically switches the index last; failure-preservation regression evidence passes. |
| R-022 | resolved | Source installs, built artifacts, API prose, and repeated version strings could describe different products or overstate the current release stage. | P7-013B establishes one metadata-backed identity, clean installed wheel/sdist tests, a guarded sdist source boundary, current public-API documentation, default-branch links, and pre-alpha language; P7-014 inventories the surface and Phase 8 alone promotes the public alpha. |
| R-023 | resolved | Native LSL discovery initially accepted generated channel names and unknown units because network resolution returns a short `StreamInfo` without its descriptor tree. | Dense discovery now retrieves full metadata through a temporary observational inlet, closes it deterministically, and the installed-wheel native smoke verifies exact labels/units plus packet transport. The real EEG gate remains separate. |

No architecture or local-code blocker is currently recorded. Phase 7 closure
is externally blocked on the real EEG observe-only acceptance record and a
fresh supported remote workflow for the published current candidate. Phases 2 through 6 provide the
...
checks pass. P7-013A adds construction-safe plugin inspection and generalized
installed-plugin conformance, including deterministic empty/all-invalid model
behavior. P7-013B adds transactional generated revisions, metadata-backed
version identity, current API/install prose, clean wheel/sdist source
boundaries, and independently visible release gates. Comprehensive validation,
broader research-integration claims,
performance/fault qualification, and public-alpha promotion remain Phase 8.
P7-014's local matrix is complete; the real EEG LSL observe-only gate and the
current-candidate remote workflow remain open.
...
| 2026-07-29 | Supported-version CI packaging correction implemented | The clean-wheel acceptance no longer disables build isolation for either distribution, so Python 3.12+ runners do not depend on an ambient legacy build backend. Checkout/setup actions move to their Node 24-compatible majors. The focused wheel journey, all 369 tests on Python 3.14 with thirteen conditional skips, compile-all, workflow parsing, and diff checks pass locally; the failed workflow smoke path was subsequently confirmed corrected and R-020 is resolved. |
| 2026-07-29 | Post-P7-013 readiness review assigned | The prior workflow smoke failure is treated as corrected. P7-013A now includes invalid-input model conformance; new P7-013B owns transactional project publication, single-source release identity, current API/install documentation, built-artifact and release-workflow evidence, and truthful pre-alpha naming. P7-014 consumes those proofs. Phase 8 receives the public `validate` aggregator, support-qualified MNE/research integrations, scale/fault qualification, TestPyPI rehearsal, and public-alpha promotion; no new top-level phase is introduced. |
| 2026-07-29 | P7-013A plugin conformance and P7-013B release/project integrity completed | Construction-free plugin inspection and explicit reusable behavioral checks now cover an independently installed transform plus valid, empty, partially invalid, and all-invalid example-model inputs, cleanup, state restoration, and deterministic fresh-component replay. Compiles publish a complete content-addressed generated revision and atomically switch the project index last; failure preserves prior bytes. Metadata supplies Python/CLI version identity, current docs and pre-alpha naming agree, and independently visible release jobs cover wheel, sdist, YAML/live imports, external plugins, and reference projects. A guarded `MANIFEST.in` closes a discovered sdist-only leak of `eegle.ml` and tests; the corrected sdist and wheel rebuilt from it exclude migration-only sources, pass Twine, and the sdist installs cleanly with version and negative-import checks. All 378 Python 3.14 tests pass, including the isolated wheel/sdist/plugin journey, plus compile-all, focused Ruff, workflow YAML parsing, and diff checks. P7-010 real EEG acceptance and the P7-014 gate matrix remain open. |
| 2026-07-29 | P7-014 local closure audit completed; external gates retained | All fifteen Phase 7 exit gates and eleven closure requirements now map to durable human and machine evidence. Clean artifact acceptance spans wheel, sdist, independent plugin, and all six references. A native `pylsl 1.18.2` installed-wheel probe exposed resolver-short channel metadata and now verifies full `C3`/`C4` labels, `uV` units, and dense packet transport through a temporary full-info inlet. The non-publishing package dispatch is separate from the false-by-default Phase 8 TestPyPI input. All 385 Python 3.14.4 tests pass, plus the native probe, compile-all, focused Ruff, workflow/evidence parsing, and diff checks. Phase 7 remains externally blocked on the non-participant real EEG observe-only record and current-candidate `tests`/non-publishing `package` workflow results. |
| 2026-08-01 | Phase 8 correctness, live hardening, and research-integration slices implemented | Versioned validation schemas and five-state results, a typed compile/runtime metric registry, fail-closed zero-window coverage, current semantic evidence decoding, schema-derived replay taxonomy with zero-record refusal, real action-request inspection identity, and the read-only `validate` Python/CLI path are implemented. Locked runs stream durable evidence, publish unsupervised interruption evidence, and support supervisor-authorized recovery at the verified ledger boundary after process loss. LSL preserves raw timestamps plus recorded mappings and has compiled engine coverage for silence/reconnect/gaps/cancellation/duration/persistence and source-health validation. Dependency-lazy MNE adapters cover regular/explicit raw, marker annotations, admitted-window epochs, and separate raw-plus-annotation replay inputs. The packaged support matrix keeps representable, adapter-available, validated, and reference-supported claims independent. All 422 repository tests pass on Python 3.14, including isolated wheel/sdist/plugin/reference and installed `validate` journeys; compile-all, focused Ruff, and diff checks pass. The expanded native LSL/MNE workflow result and real EEG record remain external evidence. |
| 2026-08-01 | First P8-003 performance and fault qualification implemented | A packaged, typed, hash-verified profile publishes explicit compact-engine memory/throughput/latency, durable incremental capture, and queue-accounting budgets plus expected outcomes for truncation, corruption, missing artifacts, source silence/reconnect/gaps, clock drift, and reject-newest backpressure. Profiles demonstrated linear retained phase results and heap-wide queue accounting; bounded result retention/incremental capture and maintained queue counters correct those two boundaries without splitting the semantic coordinator. Per-mapping drift validation prevents unrelated clocks from creating false drift. All 431 repository tests pass, including the isolated wheel/sdist journey after its pinned build backend was available; compile-all and focused Ruff pass. High-channel/sparse, process-proxy, repeated-fault, and multi-process restart breadth remains. |
| 2026-08-17 | Workbench-controlled live lifecycle and Study 1 demo path implemented | Provisional operations-owned `RunControl` adds first-request-wins graceful completion/cancellation without exposing the engine. Open LSL completion drains queued work, closes sources/writers, persists typed request/reason evidence, registers cancelled sessions, and replays equivalently against captured sources. The separate Workbench demo now supports deterministic simulation through compile/preflight/run/inspect/replay and software-complete PsychoPy/Neuracle orchestration; real Neuracle/display acceptance remains external and P7-010 stays in progress. |
