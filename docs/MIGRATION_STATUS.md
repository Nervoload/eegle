...
| Phase status | Active; P7-014 local closure audit is complete, while real EEG acceptance and current-candidate remote workflow evidence remain pending |
...
| P7-010 | in progress | Implement the optional first-party LSL source/event/outlet and discovery path. | Dense/sparse/metadata sources and outlets, exact detection/selectors, full native channel-descriptor retrieval, clock sync, reconnect, loss evidence, blocked-import isolation, simulated portability, and installed-wheel native local-network packet acceptance pass. Truthful support remains `simulated_validated` until the documented real EEG observe-only run is performed. |
...
| P8-001 | todo | Implement comprehensive validation as a public read-only service and `eegle validate` command. | One structured aggregator reports schema/compiler, evidence, timing, causality, model, adaptation, action, replay, and protocol results with explicit insufficient-evidence status and no second validation authority. |
| P8-002 | todo | Qualify research integrations and publish the support matrix. | MNE annotations/epochs/replay input and any pyRiemann or foundation-model path are promoted only with installed-artifact acceptance; every integration is labeled representable, adapter-available, validated, or reference-supported. |
| P8-003 | todo | Complete performance, scale, and fault qualification. | Published throughput/memory/latency/backpressure/channel/event budgets plus crash, truncation, corruption, drift, and missing-source probes; any `runtime/graph.py` extraction is evidence-driven and preserves the sole ordering authority. |
