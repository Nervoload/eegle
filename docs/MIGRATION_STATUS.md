...
| R-020 | monitoring | The supported-version GitHub matrix reached all 369 tests but Python 3.12/3.13 jobs failed the clean-wheel journey because it disabled PEP 517 build isolation and relied on ambient `setuptools`; the lone Python 3.11 pass was environment-dependent. | Both base and example-plugin wheel builds now provision the exact backends declared in their `pyproject.toml` files, and the workflow actions target their Node 24-compatible majors. The clean-wheel test and full 369-test suite pass locally; retain this risk until the corrected remote matrix is green. |
